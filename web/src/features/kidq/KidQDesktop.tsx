...
export default function KidQDesktop() {
  ...
  if (stage === "profile") return <Shell label="Who's watching today?" bare><section className={styles.profileScreen}><div className={styles.profileSky} aria-hidden="true">{PROFILE_STARS.map((s, i) => <span key={i} className={styles.skyStar} style={starStyle(s)} />)}<CloudBig className={styles.skyCloud} top="16%" /><CloudSmall className={styles.skyCloudSmall} top="36%" /></div><h1>Who&apos;s watching<br />today?</h1>{children.length > 0 ? <div className={styles.profileChoices}>{children.map((item, index) => <button key={item.id} onClick={() => chooseChild(item)}><span className={styles.faceWrap} style={{ width: 72, height: 72 }}><span className={styles.faceHalo} style={{ animationDelay: `${-(index % 2) * 2.2}s` }} /><AvatarFace disc={CHILD_COLOURS[index % CHILD_COLOURS.length]} face={FACE_INK[index % FACE_INK.length]} /></span><b>{item.nickname}</b></button>)}</div> : childrenLoaded && <div className={styles.noProfiles}><p>No child profiles yet.</p><Link href="/parent">Set up a child profile in Parent view</Link></div>}</section></Shell>;
