...
export default function KidQDesktop() {
  ...
  const jingleRef = useRef<HTMLAudioElement>(null);
  const playJingle = () => jingleRef.current?.play().catch(() => {});
  ...
  if (stage === "sunrise") return <Shell label={`${childName}'s session`}><section className={styles.sunriseScreen}><audio ref={jingleRef} src="/kid-prototype/proposal-src/splash-jingle.mp3" preload="auto" /><div className={styles.sunriseSky}><span className={styles.sunriseStars} /><div className={styles.sunriseCenter}><h1>Hi,<br />{childName}!</h1><button className={styles.sunButton} onClick={() => { playJingle(); startWatching(0); }} aria-label="Start today's watching session"><Sun /></button><p>Tap the sun to start your day</p><p className={styles.heartLine}><Heart /> <b>Mumma &amp; Papa picked {queue.length} video{queue.length === 1 ? "" : "s"}</b> · {plannedMinutes} min</p></div></div></section></Shell>;
