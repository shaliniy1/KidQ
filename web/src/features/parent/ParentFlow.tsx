...
import { getLibrary, removeFromLibrary, submitVideo, type LibraryEntry } from "@/services/my-videos";
...
function toParentRecommendationCard(card: Recommendation["card"], selectedAge: string, reason: string): ParentRecommendation {
  return {
    id: card.id,
    title: card.title,
    // Never 0: a video under 30s would otherwise round down to "0 min", which reads as
    // broken to a parent. Matches the floor already used for the Q-total calculation
    // on the standalone recommendations page (app/recommendations/[childId]/page.tsx).
    duration: Math.max(1, Math.round((card.duration_seconds ?? 0) / 60)),
...
export default function ParentFlow() {
  ...
  async function loadLibrary() {
    if (child) {
      try {
        ...
        // Keep the full entry (title, thumbnail, state) — a prior version of this
        // screen kept only { id, title }, which is why every row rendered the same
        // generic icon and the exact same static caption regardless of the real
        // content behind it (QA-reported "identical" rows / purple placeholder).
        setLibrary(entries);
      } catch {
  ...
  function removeFromLibraryList(id: string) {
    setLibrary((items) => items.filter((item) => item.card.id !== id));
  ...
  return <main className={styles.app}>
    ...
    <div className={screen === "login" ? `${styles.shell} ${styles.shellLogin}` : styles.shell}>
      ...
      {screen === "home" && child && <Home child={child} active={active} children={children} duration={duration} timeMode={timeMode} setTimeMode={setTimeMode} chooseChild={chooseChild} setDuration={setDuration} saveDuration={saveDuration} getRecommendations={loadRecommendations} setScreen={navigateScreen} parentName={parentName} />}
      ...
      {screen === "library" && child && <Library library={library} remove={removeFromLibraryList} add={() => navigateScreen("add")} />}
...
function Home({ child, active, children, duration, timeMode, setTimeMode, chooseChild, setDuration, saveDuration, getRecommendations, setScreen, parentName }: { child: Child; active: number; children: Child[]; duration: number; timeMode: string; setTimeMode: (v: string) => void; chooseChild: (i: number) => void; setDuration: (v: number) => void; saveDuration: (v?: number) => void; getRecommendations: () => void; setScreen: (s: Screen) => void; parentName: string }) { const [showTiming, setShowTiming] = useState(false); return <>
...
function Details({ child, library, back }: { child: Child; library: LibraryEntry[]; back: () => void }) { return <div className={styles.flow}><Header title={`${child.name}'s session log`} sub="Factual — what played, nothing about mood or attention." back={back} />{library.map((item) => <div className={styles.listRow} key={item.card.id}><span>🎬</span><div><b>{item.card.title}</b><small>Approved catalog · 7 min</small></div><span>👍</span></div>)}<p className={styles.fine}>Want to stop a video from being suggested again? Manage that from My Videos.</p></div>; }
...
function LibraryThumb({ title, thumbnailUrl }: { title: string; thumbnailUrl?: string | null }) {
  const [broken, setBroken] = useState(false);
  if (!thumbnailUrl || broken) return <span className={styles.libraryIcon} role="img" aria-label={`${title} (no thumbnail available)`}>🎬</span>;
  return <img className={styles.libraryThumb} src={thumbnailUrl} alt={`${title} thumbnail`} onError={() => setBroken(true)} />;
}
// Real per-item state, not a caption that would read the same for every row regardless of
// what's actually behind it — QA flagged two entries as "identical" partly because of that.
function libraryCaption(item: LibraryEntry): string {
  return item.state === "REQUESTED" ? "Pending KidQ review" : "In your library";
}
function Library({ library, remove, add }: { library: LibraryEntry[]; remove: (id: string) => void; add: () => void }) { return <div className={styles.flow}><Header title="My Videos" sub="Everything your family can watch, from every source." />{library.length === 0 && <p className={styles.fine}>No videos yet — add one below.</p>}{library.map((item) => <div className={styles.listRow} key={item.card.id}><LibraryThumb title={item.card.title} thumbnailUrl={item.card.thumbnail_url} /><div><b>{item.card.title}</b><small>{libraryCaption(item)}</small></div><button onClick={() => remove(item.card.id)}>Remove</button></div>)}<Button onClick={add}>＋ Add a video</Button></div>; }
