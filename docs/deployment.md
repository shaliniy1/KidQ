...
- **Gemini free**: each model has its own daily request limit, shown in Google AI Studio → *Rate limit*. For `gemini-3.8-flash` it is only **20 requests a day** (measured 2026-09-12), and YouTube input is capped at 8 hours of video a day. KidQ moves from 3.8 to 3.7 to 3.6 Flash as each runs out (`AI_FALLBACK_MODELS`) — about 60 items a day — and when all are used up it pauses until midnight Pacific, then resumes on its own. Turning on billing lifts the limits at about 2¢ an item.
...
5. **Check**: `https://<kidq-api>/ready` lists the migrations `001`–`005`; `/docs` shows every endpoint.

## Seed the catalogue (~270 videos, ~48 picture books)
...
- **Saved before the Gemini key was set?** On the admin dashboard, click **Score N items with AI**.
- **StoryWeaver** books come from the same plan; see [`content-curation/storyweaver.md`](content-curation/storyweaver.md) for what is stored and the open items before prod.
...
1. `/ready` is green.
2. Sign in to the admin app. The library lists items with their states; a detail page plays the video in the KidQ Player and shows the AI scores with evidence. A StoryWeaver book opens in the story reader, with its credits after the last page.
