...
- **API**
  - ingestion from YouTube, NASA, Wikimedia and StoryWeaver (picture books), with rule pre-checks and the Gemini scoring agent for videos and books
  - KidQ content score (`KIDQ_SCORE_V1`) and the admin gate; books are scored on three components (no audio)
  ...
  - one vocabulary for onboarding and admin tagging: 5 age bands and the 12 onboarding categories (`docs/recommendation/parent-onboarding.md`)
  - recommendations (`RANK_V1`), the parent library, and parent URL submissions
  - OpenAPI contract at `/openapi.json` and `/docs`
- **Admin Content Studio** (`admin/`) and the shared **KidQ Player** and **story reader** (`packages/kidq-player`).
- **Deploy**:
...
- 47 API tests pass: unit, plus integration tests on Postgres using fixtures.
- Live pulls into local Postgres: 49 NASA and 30 Wikimedia videos, and 45 openly licensed StoryWeaver books.
- Live Gemini scoring (`gemini-3.8-flash`) works end to end: real component scores with evidence and timestamps. When the daily quota ran out, the remaining items waited instead of failing.
...
- **Not yet run against real services**: YouTube import. It needs the YouTube key.
...
- **Gemini free tier**: `gemini-3.8-flash` allows only 20 requests a day per project (measured from Gemini's own error). Each model has its own quota.
  - Decision (2026-09-12): stay free and rotate 3.8 → 3.7 → 3.6 Flash, about 60 items a day, so the current backlog takes about 2 days. Billing (about 2¢ an item) would score everything at once.
...
- **16 NASA videos can't be AI-reviewed**: 12 credit people outside NASA, so their rights are unknown; 4 run longer than 20 minutes. Admins rate or reject them.
...
1. Parent-app onboarding screens on the new API (teammate). Then activities, the session timer and breaks, child player APIs, TV pairing, PWA install.
2. AI calibration: compare against ~30 hand-scored items before relying on bulk approval, and add score anchors to the prompt so models agree.
3. Openverse and Internet Archive connectors.
