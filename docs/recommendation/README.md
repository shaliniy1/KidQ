...
- **What it sees**
  ...
  - Only the video, its public metadata and the rubric are sent — never any parent or child data.
...
- **Free-tier guard**: at most 7.5 hours of YouTube video per day (`AI_DAILY_VIDEO_SECONDS_CAP`), against Google's 8-hour free limit. It resumes after midnight Pacific. Parent requests are scored first.
...
2. **Hard filters**: the child's age is within `[age_min, age_max]`, the language matches, the content type matches if the parent chose any, and the item isn't already in the library or recently dismissed.
...
4. **Rank** = 0.45·relevance + 0.35·(score/100) + 0.10·expert (0.5 when there are no reviews) + 0.10·preference (duration fits the daily minutes).
...
- Age groups are **0–2, 2–4, 4–6**. Items store `age_min` / `age_max`, and groups are derived from them.
...
| Age | 0–2 / 2–4 / 4–6 | One-year bands | 0–2 / 2–4 / 4–6, stored as min/max |
| Filters vs ranking | — | Interests and goals as filters | Hard filters: age, language, type. Interests and goals rank (hard AND-filters empty a 300-item catalogue) |
